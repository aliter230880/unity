...
namespace AliTerra
{
    ...
    public class AliTerraAICoder : EditorWindow
    {
        ...
        void FlushLogs()
        {
            ...
            SendPost(SERVER_URL + "/api/unity/logs", json, null);
        }
        ...
        {
            if (string.IsNullOrEmpty(SERVER_URL)) yield break;
            string json = BuildStateJson();
            yield return SendPostRoutine(SERVER_URL + "/api/unity/push", json, null);
        }
        ...
        IEnumerator SyncRoutine()
        {
            ...
            yield return SendPostRoutine(SERVER_URL + "/api/unity/sync", payload, (ok, text) =>
            {
                syncBusy  = false;
                if (ok)
                {
        ...
        IEnumerator SendChatRoutine(string json)
        {
            ...
            yield return SendPostRoutine(SERVER_URL + "/api/ai/chat", json, (ok, text) =>
            {
                success      = ok;
                responseText = text;
            });
        ...
        void DrawFullstackTab()
        {
            ...
            // Server
            EditorGUILayout.LabelField("Сервер: " + SERVER_URL, EditorStyles.miniLabel);
        ...
        void DrawDebugTab()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("Сервер: " + SERVER_URL);
